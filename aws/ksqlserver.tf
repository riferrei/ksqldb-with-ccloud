...
data "template_file" "ksqldb_server_container_def" {
  template = file("ksqldb-server.json")
  vars = {
...
resource "aws_ecs_task_definition" "ksqldb_server_task_def" {
  family = "ksqldb_server_task_def"
  network_mode = "awsvpc"
  ...
  container_definitions = data.template_file.ksqldb_server_container_def.rendered
}
...
resource "aws_ecs_service" "ksqldb_server_service" {
  ...
  task_definition = aws_ecs_task_definition.ksqldb_server_task_def.arn
...
resource "aws_appautoscaling_target" "ksqldb_server_auto_scaling_target" {
  count = var.automatic_scaling ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "ksqldb_server_cpu_high_alarm" {
  ...
  alarm_actions = [aws_appautoscaling_policy.ksqldb_server_auto_scaling_up[0].arn]
}
...
resource "aws_cloudwatch_metric_alarm" "ksqldb_server_cpu_low_alarm" {
 ...
 alarm_actions = [aws_appautoscaling_policy.ksqldb_server_auto_scaling_down[0].arn]
}
